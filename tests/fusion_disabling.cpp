...
#define CELERITY_STD_DISABLE_FUSION
...
SCENARIO("Fusing two tasks", "[fusion::simple]")
{
    ...
    GIVEN("Two simple transform kernels and a buffer of hundred 1s")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = transform<class add_36>(add_5);
            auto t2 = transform<class mul_37>(mul_3);
            ...
            THEN("kernels are fused and the result is 18")
            {
                ...
                static_assert(size_v<linked_t<seq_type>> == size_v<fused_t<seq_type>>);
    ...
    GIVEN("Two simple transform kernels and a buffer of hundred 1s also taking the current item")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = transform<class add_w_item_66>(add_5);
            auto t2 = transform<class mul_w_item_67>(mul_3);
    ...
    GIVEN("A generate and a transform kernel")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = generate_n<class gen_item_id_93>(cl::sycl::range<1>{size}, gen_i);
            auto t2 = transform<class mul_3_f_94>(mul_3);
    ...
    GIVEN("A fill value and a transform kernel")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = fill_n<class fill_1_124>(cl::sycl::range<1>{size}, init);
            auto t2 = transform<class mul_3_f_125>(mul_3);
    ...
    GIVEN("A fill value and two simple transform kernels")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = fill_n<class fill_152>(cl::sycl::range<1>{size}, init);
            auto t2 = transform<class add_w_item_153>(add_5);
            auto t3 = transform<class mul_w_item_154>(mul_3);
    ...
    GIVEN("A generate and three transform kernels")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = generate_n<class generate_item_id_182>(cl::sycl::range<1>{size}, gen_i);
    ...
    GIVEN("A generate kernel, a buffer and a zip kernel")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = generate_n<class generate_item_id_219>(cl::sycl::range<1>{size}, gen_i);
            auto t2 = transform<class zip_add_t_220>(zip_add);
    ...
    GIVEN("Two generate kernels and a zip kernel")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = generate_n<class generate_item_id_251>(cl::sycl::range<1>{size}, gen_i);
            auto t2 = fill_n<class fill_3_252>(cl::sycl::range<1>(size), 3);
    ...
    GIVEN("Two generate kernels a zip kernel and a transform kernel")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = generate_n<class generate_item_id_2_287>(cl::sycl::range<1>{size}, gen_i);
            auto t2 = fill_n<class fill_4_288>(cl::sycl::range<1>(size), 3);
            auto t3 = transform<class zip_add_t_2_289>(zip_add);
            auto t4 = transform<class mul_2_t_1_290>(mul_2);
    ...
    GIVEN("Two generate kernels a zip kernel and two transform kernel")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = generate_n<class generate_item_id_322>(cl::sycl::range<1>{size}, gen_i);
    ...
    GIVEN("One generate kernel, a buffer and a zip kernel")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = generate_n<class generate_item_id_3_359>(cl::sycl::range<1>{size}, gen_i);
            auto t3 = transform<class zip_add_t_3_360>(zip_add);
            auto seq = t1 | (t3 << buf);
            auto buf_out = seq | submit_to(q);
    ...
    GIVEN("One generate kernel, a buffer and a zip kernel")
    {
        ...
        WHEN("chaining calls")
        {
            auto t1 = generate_n<class generate_item_id_392>(cl::sycl::range<1>{size}, gen_i);
    ...
    GIVEN("One generate kernel, a buffer and a zip kernel")
    {
        ...
        buffer<int, 1> buf_a{{size}};
        buffer<int, 1> buf_b{{size}};
        fill<class _25>(q, begin(buf_a), end(buf_a), 1);
        fill<class _26>(q, begin(buf_b), end(buf_b), 3);
    ...
    GIVEN("One generate kernel, a buffer and a zip kernel")
    {
        ...
        buffer<int, 1> buf_a{{size}};
        buffer<int, 1> buf_b{{size}};
        fill<class _27>(q, begin(buf_a), end(buf_a), 1);
        fill<class _28>(q, begin(buf_b), end(buf_b), 3);

        WHEN("chaining calls")
        {
            auto t2 = transform<class mul_chunk_27>(mul_chunk);
            auto t3 = transform<class zip_add_t_28>(zip_add);
            auto t4 = transform<class zip_add_sec_5>(zip_add_sec);
    ...
    GIVEN("One generate kernel, a buffer and a zip kernel")
    {
        ...
        buffer<int, 1> buf_a{{size}};
        buffer<int, 1> buf_b{{size}};
        fill<class _29>(q, begin(buf_a), end(buf_a), 1);
        fill<class _30>(q, begin(buf_b), end(buf_b), 3);

        WHEN("chaining calls")
        {
            auto t2 = transform<class mul_chunk_29>(mul_chunk);
            auto t3 = transform<class zip_add_t_30>(zip_add);
            auto t4 = transform<class zip_add_sec_6>(zip_add_sec);
    ...
    GIVEN("One generate kernel, a buffer and a zip kernel")
    {
        ...
        buffer<int, 1> buf_a{{size}};
        buffer<int, 1> buf_b{{size}};
        fill<class _31>(q, begin(buf_a), end(buf_a), 1);
        fill<class _32>(q, begin(buf_b), end(buf_b), 3);

        WHEN("chaining calls")
        {
            auto t2 = transform<class mul_chunk_31>(mul_chunk);
            auto t3 = transform<class zip_add_t_32>(zip_add);
            auto t4 = transform<class zip_add_sec_7>(zip_add_sec);
            auto seq = buf_a | t4 << (buf_b | t2) | t3 << (buf_a | t2);
