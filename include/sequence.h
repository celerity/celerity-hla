...
namespace celerity::hla
{
	...
	{
		template <typename... Actions>
		struct sequence_traits<hla::detail::sequence<Actions...>> : std::integral_constant<bool, true>
		{
		};

		template <typename T, typename... Ts>
		struct last_element<hla::detail::sequence<T, Ts...>>
			: std::tuple_element<hla::detail::sequence<T, Ts...>::num_actions - 1, typename hla::detail::sequence<T, Ts...>::actions_t>
		{
		};

		template <>
		struct last_element<hla::detail::sequence<>>
		{
			using type = void;
		};

		template <typename T, typename... Ts>
		struct first_element<hla::detail::sequence<T, Ts...>> : std::tuple_element<0, typename hla::detail::sequence<T, Ts...>::actions_t>
		{
		};

		template <>
		struct first_element<hla::detail::sequence<>>
		{
			using type = void;
		};

		template <size_t Idx, typename... Ts>
		struct nth_element<Idx, hla::detail::sequence<Ts...>> : std::tuple_element<Idx, typename hla::detail::sequence<Ts...>::actions_t>
		{
		};

		template <size_t Idx>
		struct nth_element<Idx, hla::detail::sequence<>>
		{
		};
