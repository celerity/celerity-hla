...
inline constexpr bool
    single_element_access_v = (computation_type_of_v<T, detail::computation_type::transform> && access_type_v<T> == detail::access_type::one_to_one)
                              || (computation_type_of_v<T, detail::computation_type::zip> && access_type_v<T> == detail::access_type::one_to_one
...
constexpr inline bool first_input_stage_completed_v =
    is_packaged_task_v<
        T> || (computation_type_of_v<T, algorithm::detail::computation_type::zip> && !std::is_void_v<typename extended_packaged_task_traits<T, algorithm::detail::computation_type::zip>::second_input_iterator_type>)
    || stage_requirement_v<T> == detail::stage_requirement::output;
