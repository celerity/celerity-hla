...
namespace celerity::algorithm::detail
{
	...
	namespace link_impl
	{
		...
		auto link_internally(T t_joint)
		{
			...
			if constexpr (is_celerity_buffer_v<decltype(last)> || is_buffer_range_v<decltype(last)>)
			{
				if constexpr (size_v<decltype(secondary)> == 1)
				{
					return t_joint.get_task().complete(begin(last), end(last));
				}
				else
				{
		...
		auto operator+(T lhs, U rhs)
		{
			auto t_right = link_internally(rhs).complete(lhs.get_out_beg(), lhs.get_out_beg());
	...
	auto link(const sequence<Actions...> &s)
	{
		...
		return ensure([&]() {
			...
			{
				return detail::sequence(link_internally(get_first_element(seq)));
			}
			else
			{
