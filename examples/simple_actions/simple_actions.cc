	...
	try
	{
...

		{
			auto sum =
				...
				actions::transform(distr<class compute_d_1>(), begin(buf_b), end(buf_b), begin(buf_c), begin(buf_d), [](const float x, const float y) { return x + y; }) |
				actions::accumulate(master_blocking(queue), begin(buf_d), end(buf_d), 0.0f, [](const float acc, const float x) { return acc + x; }) |
				submit_to(queue);
...

		{
			...
			auto compute_d = actions::transform(distr<class compute_d_2>(), begin(buf_b), end(buf_b), begin(buf_c), begin(buf_d), [](const float x, const float y) { return x + y; });
			auto reduce_d = actions::accumulate(master_blocking(queue), begin(buf_d), end(buf_d), 0.0f, [](const float acc, const float x) { return acc + x; });
